...
void ArbGenSystem::initialize(std::string headerText, IChimeraQtWindow* win)
{
	...
	siglentFmCtrlButton = new CQCheckBox("Control", win);
...
double ArbGenSystem::getPulseDurationMs() const {
	bool ok = false;
	double durationMs = ch1PulseDurationMsEdit ? ch1PulseDurationMsEdit->text().toDouble(&ok) : 0;
	if (!ok || durationMs <= 0) {
		return 0;
	}
	return durationMs;
...
void ArbGenSystem::updateCalculatedSampleRateDisplay() {
	...
	double durationMs = getPulseDurationMs();
	if (durationMs <= 0) {
