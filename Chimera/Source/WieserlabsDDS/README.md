...
- Both channels execute **simultaneously** when triggered on their respective BNC inputs

## Trigger Configuration
**Hardware-triggered via FlexDDS-NG external inputs** (not software-driven):
- **Channel 0:** `(1, 0)` → **BNC_IN_A** on DDS hardware  
- **Channel 1:** `(1, 1)` → **BNC_IN_B** on DDS hardware
...
dcp <channel> wait::BNC_IN_B_RISING   # for Channel 1
```
...
- **Hardware trigger:** ✓ **HARDWARE-DRIVEN** — Each channel independently waits for physical TTL on BNC_IN_A (Ch0) or BNC_IN_B (Ch1).
...
3. Issue hardware trigger arm command: `dcp <channel> wait::BNC_IN_A_RISING` or `wait::BNC_IN_B_RISING`.
4. Wire TTL lines to the DDS:
   - Channel 0 script: TTL line `(1, 0)` → DDS **BNC_IN_A**
   - Channel 1 script: TTL line `(1, 1)` → DDS **BNC_IN_B**
